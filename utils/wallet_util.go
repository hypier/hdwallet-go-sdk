...
func EthGetPrivateKeyFromWif(wif string) (*ecdsa.PrivateKey, error) {
	pri, err := crypto.HexToECDSA(wif)
	if err != nil {
		return nil, err
	}
	return pri, nil
...
func BtcGetPrivateKeyFromWif(wif string) (*btcec.PrivateKey, error) {
	btcWif, err := btcutil.DecodeWIF(wif)
	if err != nil {
		return nil, err
	}
	return btcWif.PrivKey, err
}
...
func createVinList(mtx *wire.MsgTx) []btcjson.Vin {
	// transactions only have a single txin by definition.
