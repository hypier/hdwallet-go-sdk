...
// NewReverseResolverForA creates a reverse resolver contract for the given address.
func NewReverseResolverForA(backend bind.ContractBackend, contract common.Address, address common.Address) (*ReverseResolver, error) {
	registry, err := NewRegistryAt(backend, contract)
	if err != nil {
		return nil, err
	}
	...
	return NewReverseResolverAtA(backend, contractAddress)
}
...
// NewReverseResolverAt obtains the reverse resolver at a given address
func NewReverseResolverAtA(backend bind.ContractBackend, address common.Address) (*ReverseResolver, error) {
	// Instantiate the reverse registrar contract
	contract, err := reverseresolver.NewContract(address, backend)
	if err != nil {
		return nil, err
